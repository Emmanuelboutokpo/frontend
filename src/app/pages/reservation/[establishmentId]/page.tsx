...
export default function ReservationPage() {
    ...
    if (!property) return <main className="mx-auto max-w-3xl px-4 py-24 text-center"><h1 className="text-2xl font-bold">Réservation introuvable</h1><Button asChild className="mt-6 rounded-full bg-emerald-700"><Link href="/pages/">Retour à l’accueil</Link></Button></main>
    if (!isAuthenticated) return <main className="mx-auto max-w-3xl px-4 py-24 text-center"><LockKeyhole className="mx-auto h-10 w-10 text-emerald-700" /><h1 className="mt-5 text-2xl font-bold">Connectez-vous pour réserver</h1><p className="mt-3 text-slate-500">Votre réservation sera associée à votre compte.</p><Button asChild className="mt-6 rounded-full bg-emerald-700"><Link href={`/pages/${property.category_id === 2 ? 'restaurant' : property.category_id === 3 ? 'loisir' : 'hebergement'}/${property.slug}`}>Retour à l’établissement</Link></Button></main>
    ...
    return <main className="min-h-screen bg-[#ffff] pb-20 text-slate-900"><div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-12">
        <Link href={`/pages/${property.category_id === 2 ? 'restaurant' : property.category_id === 3 ? 'loisir' : 'hebergement'}/${property.slug}`} className="inline-flex items-center gap-2 text-sm font-semibold text-slate-600 hover:text-emerald-700"><ArrowLeft className="h-4 w-4" />Retour à l’établissement</Link>
