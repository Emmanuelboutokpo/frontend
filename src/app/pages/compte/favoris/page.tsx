                ...
                <div className="mt-16 rounded-3xl border border-emerald-100 bg-white px-6 py-16 text-center shadow-sm">
                    ...
                    {countryGroups.map(({ country, properties }) => {
                        ...
                        return <section key={country.id} aria-labelledby={`country-${country.id}`}>
                            ...
                            <div className="mt-6 space-y-8">{categories.map(({ category, properties: categoryProperties }) => <div key={category.id}><div className="mb-4 flex items-center justify-between sm:justify-start gap-3"><h3 className="text-lg font-bold">{category.name}</h3><span className="text-sm text-slate-400">{categoryProperties.length} établissement{categoryProperties.length > 1 ? 's' : ''}</span></div><div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{categoryProperties.map((property) => <PropertyCard key={property.id} property={property} countryName={country.name} photo={photos[property.id]} subcategorySlugs={subcategorySlugs} />)}</div></div>)}</div>
                        </section>
                    })}
                </div>}
            </div>
